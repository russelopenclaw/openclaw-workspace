...
async function main() {
  console.log('🎬 Generating Dad Joke Video #16\n');
  console.log(`Joke: "${JOKE_16.text}"`);
  console.log(`Format: ${JOKE_16.format}\n`);
  
  // Step 1: Generate audio
  console.log('🎙️ Step 1: Generating audio...');
  const audioFile = await generateAudio(JOKE_16.text);
  const duration = await getAudioDuration(audioFile);
  console.log(`   Audio: ${duration.toFixed(2)}s\n`);
  
  // Step 2: Generate image prompt (invention/windows theme)
  console.log('🎨 Step 2: Generating image prompt...');
  const prompt = 'cartoon house with large transparent windows, whimsical style, bright daylight, family-friendly illustration';
...
async function generateImage(prompt) {
  const filename = `bg-16-${Date.now()}.png`;
  const filepath = path.join(CONFIG.outputDir, filename);
...
async function renderVideoSetupPunchline(joke, audioFile, imageFile, duration) {
  const videoFile = path.join(CONFIG.outputDir, `dadjoke-16-V1.mp4`);
  ...
  return new Promise((resolve, reject) => {
    const cmd = `cd ${CONFIG.remotionProject} && ./node_modules/.bin/remotion render "DadJokeVideo" "DadJokeVideo" "out/dadjoke-16-V1.mp4" --props-file="${propsFile}" --public-dir=public --duration-in-frames=${frames} --fps=30 --width=720 --height=1280`;
    
    exec(cmd, { timeout: 300000 }, (err) => {
      try { fs.unlinkSync(propsFile); } catch {}
      if (err) { reject(new Error(`Remotion: ${err.message}`)); return; }
      
      const out = path.join(CONFIG.remotionProject, 'out', 'dadjoke-16-V1.mp4');
...
async function verifyWithVision(videoFile) {
  // Extract frame
  const frameFile = path.join(CONFIG.outputDir, 'j16-verify.png');
  ...
  return new Promise((resolve, reject) => {
    exec(`curl -s -X POST "${CONFIG.ollamaUrl}/api/generate" -H "Content-Type: application/json" -d @${payloadFile}`, (err, stdout) => {
      ...
      const hasJ16Words = text.includes('invention') || text.includes('Windows') || text.includes('walls');
      const hasBackgroundDesc = !text.includes('solid') || text.includes('cartoon');
      
      resolve({
        ok: hasJ16Words && hasBackgroundDesc,
        text: text.split('\n')[0].trim(),
