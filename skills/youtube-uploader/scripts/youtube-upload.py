...
def cmd_delete(args):
    ...
    try:
        ...
        error_msg = str(e)
        if "videoNotFound" in error_msg or "not found" in error_msg.lower():
            print(json.dumps({"videoId": args.video_id, "status": "not_found"}, indent=2))
        else:
            print(f"Error deleting video: {error_msg}", file=sys.stderr)
            sys.exit(1)
...
def main():
    ...
    # delete
    p_delete = sub.add_parser("delete", help="Delete a video from YouTube")
    p_delete.add_argument("--video-id", required=True, help="YouTube video ID to delete")
    p_delete.add_argument("--channel-id", default=None, help="Target channel ID")
